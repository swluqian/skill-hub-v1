...
import { getSkillById, updateSkill, getCategories } from '../../services/skills';
import '../SubmitSkill/styles.css';
...
export default function EditSkill() {
  ...
  const [form, setForm] = useState({ name: '', description: '', content: '', version: '', install_command: '', category_id: '' });
  ...
  useEffect(() => {
    ...
    Promise.all([getSkillById(skillId), getCategories()])
      .then(([skill, cats]: [Skill, Category[]]) => {
        if (user.role !== 'admin' && skill.author_id !== user.id) { navigate('/'); return; }
        setForm({
          name: skill.name,
          ...
          category_id: String(skill.category_id),
        });
        setHasIcon(!!skill.icon_url);
        setHasZip(!!skill.install_zip_url);
        setCategories(cats);
      })
      .catch(console.error)
      ...
      <div className="submit-inner">
        ...
        <form onSubmit={handleSubmit} className="submit-form">
          ...
          <div className="form-group">
            <label>zip 包</label>
            {hasZip && <small>当前 zip 包已上传</small>}
            <input type="file" accept=".zip" onChange={(e) => setZipFile(e.target.files?.[0] || null)} />
            <small>仅支持 .zip 格式，最大 50MB</small>
          </div>
          <div className="form-group">
            <label>图标</label>
            {hasIcon && <small>当前图标已上传</small>}
            <input type="file" accept="image/*" onChange={(e) => setIconFile(e.target.files?.[0] || null)} />
          </div>
          <button type="submit" className="btn btn-primary btn-block" disabled={loading}>{loading ? '保存中...' : '保存修改'}</button>
        </form>
      </div>
    </div>
  );
